...
class BaseMySQLListenerWithCheckpointMUL:
    """
    按照月进行分表的表结构数据跟踪基类
    """

    def __init__(self, host, port, user, password, database, table_prefix, query_interval=600, max_err_count=10):
        # 初始化数据库连接参数
        self.host = host
        self.port = port
        self.user = user
        ...
        self.query_interval = query_interval  # 查询间隔，默认每10秒查询一次

        # 获取当前月份并生成对应的表名和偏移文件名
        ...
        self.connection = self.create_connection()  # 创建数据库连接
        ...
        try:
            with self.connection.cursor() as cursor:
                ...
                if len(rows) == 0 and self.current_month != self.previous_month:
                    ...
                    self.previous_month = self.current_month
                    self.table_name = f"{self.table_prefix}{self.current_month}"  # 动态表名
                    self.offset_file = f"./offsetLogs/offset_{self.table_name}.txt"  # 动态偏移文件名
