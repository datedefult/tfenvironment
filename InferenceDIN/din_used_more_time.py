...
if __name__ == '__main__':
    ...
    # 提取时间信息
    current_month = now.month  # 月
    current_day = now.day  # 日
    current_hour = now.hour  # 时
    current_minute = now.minute  # 分
    current_second = now.second  # 秒
    current_weekday = weekday_mapping[now.strftime("%A")]  # 周几
    ...
    test_input = {
        ...
        'day': tf.constant([current_day] * n, dtype=tf.int64),
        'hour': tf.constant([current_hour] * n, dtype=tf.int64),
        'minute': tf.constant([current_minute] * n, dtype=tf.int64),
        'second': tf.constant([current_second] * n, dtype=tf.int64),
        'weekday': tf.constant([current_weekday] * n, dtype=tf.int64),
