...
def data_process(data_df, dense_features, sparse_features):
    ...
    for f in dense_features:
        ...
        scaler = MinMaxScaler()
        data_df[f] = scaler.fit_transform(data_df[[f]])
...
if __name__ == "__main__":
    ...
    # data = pd.read_csv('../joy_data/test_all.csv')
    data = pd.read_csv('../joy_data/test_true_hit.csv')
    data['is_hit'] = data['is_like'].clip(lower=-1, upper=1)
    data['is_like'] = data['is_like'].clip(lower=-1, upper=1)
    # 处理方式1
    # 截止到推送时累计发布时长，天
    data['till_push'] = round(((data['timestamp']/1000) -data['post_created_at']) / (3600*24),0).clip(lower=0)

    # # 处理方式2
    # # 截止到推送时累计发布时长，ms
    # data['till_push'] = data['timestamp'] - (data['post_created_at']*1000)
    ...
    # 划分dense和sparse特征
    # 类别特征做稀疏
    sparse_features = id_cols + time_cat_cols
    # 数值特征做稠密
    dense_features = num_cols
    # 数据预处理
    train_data = data_process(data, dense_features, sparse_features)
    # 预测指标
    train_data['label'] = data[ctr_tag]
    ...
    history.compile(optimizer="adam", 
                ...
                metrics=["binary_crossentropy", tf.keras.metrics.AUC(name='auc'),Precision(name="precision"), Recall(name="recall"),])

    # 将输入数据转化成字典的形式输入
    train_model_input = {name: data[name] for name in dense_features + sparse_features}

    # 调整正样本权重，平衡正负样本
    class_weight = {0: 1., 1: len(train_data[train_data['label'] == 0]) / len(train_data[train_data['label'] == 1])}
    ...
    history.fit(train_model_input,
                train_data['label'].values.reshape(-1,1),
                batch_size=64,
                epochs=20,
