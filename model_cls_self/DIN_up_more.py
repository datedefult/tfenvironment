...
if __name__ == "__main__":
    # 读取数据
    samples_data = pd.read_csv("../joy_data/DIN_data_more.csv",sep='\t',nrows=500)
    ...
    with tf.device('/GPU:0'):
        ...
        history = tf.keras.models.load_model(
            '../trained_models/saved_din_model/3',
            custom_objects={'DIN': DIN}
        )
        history.summary()
        ...
        history.fit(X_train, y_train, batch_size=64, epochs=10, validation_split=0.2, callbacks=[early_stop,] )


    ## 将模型保存成pb格式文件
    export_path = "../trained_models/saved_din_model/"
    version = "test"  # 后续可以通过版本号进行模型版本迭代与管理
    ...
    def serving_default(uid, gender, language, hit_post_id_list, hit_len, post_id, type):
        inputs = {
            'uid': uid,
            'gender': gender,
            'language': language,
